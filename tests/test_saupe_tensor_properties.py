...
def test_fitted_tensor_is_traceless_and_symmetric():
    ...
    np.testing.assert_allclose(
        np.trace(S), 0.0, atol=1e-5, err_msg="Fitted Saupe tensor must be traceless (Sxx+Syy+Szz=0)"
    )
    np.testing.assert_allclose(
        S, S.T, atol=1e-5, err_msg="Fitted Saupe tensor must be symmetric (S=S^T)"
    )

    print(
        f"✅ Saupe tensor trace = {np.trace(S):.2e} (≈0), symmetric: {np.allclose(S, S.T, atol=1e-5)}"
    )
...
def test_fitted_tensor_recovers_ground_truth():
    ...
    np.testing.assert_allclose(
        true_tensor, fitted, atol=1e-5, err_msg="Fitted tensor does not match the ground truth"
    )
